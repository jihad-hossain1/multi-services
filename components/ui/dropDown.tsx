...
export const DropdownMenuTrigger: React.FC<DropdownMenuTriggerProps> = ({
  ...
  const { isOpen, setIsOpen, setPosition, dropdownPosition } = useDropdown();
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`
        inline-flex items-center px-3 text-sm py-1 border border-gray-200 rounded-md 
        bg-background hover:bg-background focus:outline-none focus:ring-1 
        focus:ring-offset-1 focus:ring-violet-500
...
export const DropdownMenuContent: React.FC<DropdownMenuContentProps> = ({
  ...
  return (
    <div
      ref={contentRef}
      className="absolute z-50 w-56 bg-zinc-700 rounded-md shadow-lg ring-1 ring-black 
  ...
  return (
    <button
      className={`
        ...
        ${!disabled && "hover:bg-gray-600 hover:text-gray-200"}
...
export const DropdownMenuLabel: React.FC<{ children: React.ReactNode }> = ({
  ...
  return (
    <div
      className="px-4 py-2 text-sm font-medium text-gray-200"
      role="presentation"
    >
      {children}
    </div>
 ...
 * const Example = () => {
  return (
    <DropdownMenu onOpenChange={(isOpen) => console.log('Dropdown is:', isOpen)}>
      <DropdownMenuTrigger>
        Options ▼
      </DropdownMenuTrigger>
