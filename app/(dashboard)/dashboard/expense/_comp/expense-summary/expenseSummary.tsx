...
const ExpenseSummary: React.FC = () => {
  ...
  const [summaries, setSummaries] = React.useState<{
    ...
    summaryList: Summary;
    totalAmount: number;
  }>({ summary: {}, summaryList: {}, totalAmount: 0 });
  ...
  const handleFetchSummary = async ({
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/expense/summary?userId=${auth?.userId}&summaryId=${summaryId}&summary=summary&from=${fromDate}&to=${toDate}`,
        { cache: "no-store" }
      );
  ...
  useEffect(() => {
    const fetchSummaries = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(
          `/api/v1/expense/summary?userId=${
            auth?.userId
          }&group=${JSON.stringify(groupItems)}&from=${fromDate}&to=${toDate}`,
          { cache: "no-store" }
        );
        const data = await response.json();
        setSummaries(data);
    ...
    <div>
      <div className="flex flex-col md:flex-row gap-3 md:justify-between md:items-center my-4">
        <div className="flex flex-col gap-1">
          ...
          <MultiSelectDropdown
            options={summaries?.summaryList as any}
          ...
          <table className="min-w-full bg-zinc-700 divide-y divide-zinc-200">
            <thead className="bg-zinc-700">
              <tr className="bg-zinc-700/70">
                <th
                  scope="col"
                  className="px-6 py-3 text-left text-sm font-medium text-zinc-700 uppercase tracking-wider"
                >
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-lg">
          <table className="min-w-full  divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-200">
              {summaries?.summary &&
                (() => {
                  const totalAmount = Object.values(summaries?.summary).reduce(
                  ...
                  return Object.entries(summaries?.summary)
                    ?.sort(
                      ([, valueA], [, valueB]) =>
                        (valueB?.amount || 0) - (valueA?.amount || 0)
                    )
                    ?.map(([key, value], index) => {
                      const percentage =
                        ((value?.amount || 0) / totalAmount) * 100;

                      return (
                        <React.Fragment key={index}>
                          <tr
                            className="relative hover:bg-zinc-200/20 cursor-pointer"
                            onClick={() => toggleRow(index, value.id)}
                          >
            ...
            <tfoot>
              <tr>
                ...
                <td colSpan={4} className="px-6 py-4 text-sm font-semibold">
                  {summaries?.totalAmount?.toLocaleString()}
                </td>
              </tr>
            </tfoot>
          </table>
...
const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({
  ...
  return (
    <div className="relative w-full md:w-[400px]" ref={dropdownRef}>
      <button
        className="w-full bg-zinc-200/60 text-left px-4 py-2 rounded-lg shadow-md focus:outline-none flex justify-between items-center"
      ...
      >
        <span>
          {selectedOptions.length > 0
            ? selectedOptions
                .map((id) =>
                  Object.keys(options).find((key) => options[key].id === id)
                )
                .join(", ")
            : "Select Group"}
        </span>
        ...
        <div className="absolute mt-2 w-full bg-zinc-600 drop-shadow border-zinc-300 rounded-lg shadow-lg z-30 max-h-60 overflow-y-auto animate-fade-in">
          {Object.keys(options).map((key) => {
            ...
            return (
              <div
                key={key}
                className="px-4 py-2 cursor-pointer hover:bg-zinc-700 flex items-center"
