...
const UserPage = () => {
    ...
    const fetchUsers = React.useCallback(async () => {
       try {
        ...
        if(data.result){
         dispatch({type: ACTION_TYPES.FETCH_USERS_SUCCESS, payload: data?.result})
        }
        if(data.error){
         dispatch({type: ACTION_TYPES.FETCH_USERS_FAILURE, payload: data?.error})
        }
       } catch (error) {
        console.error(error)
       }
  ...
  return (
    <div>
        {state?.error && <h1 className='text-red-500 text-center'>{state?.error}</h1>}
        <div className='overflow-x-auto w-full'>
        {
            state?.loading ? <h1>Loading...</h1> : <table className='w-full border '>
            <thead>
                <tr>
            ...
            <tbody>
               {  
                    state?.users?.map((user:any) => (
                        <tr key={user?.id}>
                            <td className='px-4 py-2 text-left border-b border-gray-200'>{user?.name}</td>
                            <td className='px-4 py-2 text-left border-b border-gray-200'>{user?.email}</td>
                            <td className='px-4 py-2 text-left border-b border-gray-200'>{user?.xrole}</td>
                            <td className='px-4 py-2 text-left border-b border-gray-200'>{user?.status}</td>
                            <td className='px-4 py-2 text-left border-b border-gray-200'>{user?.count}</td>
                            <td className='px-4 py-2 text-left border-b border-gray-200'>{user?.verify}</td>
