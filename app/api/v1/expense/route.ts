...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { amount, payment, title, catid, note, xuserid, xdate, xbalance } =
      parsedData.data;

    const findBalance = await prisma.balance.findFirst({
      where: {
        xuserid: reqFromBody?.xuserid,
        id: xbalance,
      },
    });
    ...
    await prisma.balance.update({
      where: {
        xuserid: findBalance?.xuserid,
        xname: findBalance?.xname,
      },
      data: {
        amount: Number(findBalance?.amount) - Number(amount),
      },
    });
