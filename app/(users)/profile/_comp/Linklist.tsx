...
export function LinkList() {
  ...
  const deleteLink = async (id: string) => {
    try {
      ...
      if (jsondata?.result) {
        toast.success(jsondata?.result, { duration: 3000, position: 'top-right', style: { padding: '10px', borderRadius: '5px', } });
        setConfirmDLT(false);
        setDeleteLoading(false);
        closeModal();
        fetchLinks();
      ...
      if (jsondata?.error) {
        toast.error(jsondata?.error, { duration: 3000, position: 'top-right', style: { background: 'red', color: 'white', padding: '10px', borderRadius: '5px', border: '1px solid yellow' } });
      }
      ...
      >
        <div className="flex gap-2 items-center justify-center">
          <button onClick={(e) => {
            setConfirmDLT(true);
            deleteLink(linkId);
          }} className="bg-red-500 text-white px-2 py-1 rounded-md" disabled={deleteLoading}>
            {
              deleteLoading ? <Loader className="w-6 h-6 animate-spin" stroke="white" /> : "Delete"
            }
          </button>
          <button onClick={(e) => {
            setConfirmDLT(false);
            closeModal();
          }} className="bg-blue-500 text-white px-2 py-1 rounded-md">Cancel</button>
        </div>
      </DialogComponent>
    </div>
  );
